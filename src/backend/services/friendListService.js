...
async function fetchFriendsForProfilePage(ownerID) {
  try {
    const client = await pool.connect();
    const query = `
    SELECT CONCAT(own.firstName, ' ', own.lastName) AS owner_name
    FROM Friendship f JOIN Owner_Name own ON f.ownerID2 = own.ownerID
    WHERE f.ownerID1 = ${ownerID}
    ORDER BY f.dateOfFriendship DESC;
    `;
